...
`DevGov` 是 user/home-level 的治理工具包與儀表板。它把 Global AGENTS 規劃、開發服務、本機服務 Agents、shared resource coordination、Windows Terminal profiles、啟動項目、Cloudflare 對外路由、開發用 API key 存放位置、AGENTS 指令治理、worktrees、自我檢測與本機文件檢索放在可稽核的真相來源下集中管理。

此 repository 應放在 user/home-level 的 Codex storage，而不是單一 workspace project tree。它負責 global-layer planning、indexing、validation 與 coordination；它不會靜默改寫 live Global AGENTS，也不會覆寫 platform/runtime 指令。

本專案採用類似 UniText 的資料管理分層：

- `registry/` 存放 canonical shared data，是跨專案唯一 source of truth。
- `templates/` 存放可重複套用到目標專案的可讀取模型（read-model）文件與模板。
- `scripts/` 存放驗證、掃描、稽核指令。
- `reports/` 存放產生出的本機證據；除了 `.gitkeep` 以外預設不納入版控。

本工具包以 audit-first 為原則。掃描只產生 `reports/` 證據；經審查（review）的紀錄才會提升到 `registry/`。

## 重要文件

- [README.md](README.md)
- [AGENTS.md](AGENTS.md)：本 repo 唯一 authoritative 的 agent-runtime 指令來源
- [AGENTS.zh-tw.md](AGENTS.zh-tw.md)：僅供人類閱讀，非 runtime source
- [PRODUCT.md](PRODUCT.md)：Design 與 UI agents 使用的產品脈絡
- [DESIGN.md](DESIGN.md)：DevGov design system；[registry/design-system.registry.json](registry/design-system.registry.json) 是可重複使用的 token sidecar
...
稽核開發用 API key 名稱與 storage scopes，但不印出值：
...
當觸發來源是 alert text 或 failed command output 時，改用 Codex hook wrapper：
...
任何 explicit memory-update workflow 之前，先 review：

```text
templates/CODEX.memory.rcg-update-gate.md
```

Review 後，將精確 proposal payload 交由 `memory-field` 或 runtime-owned memory update architecture 處理。DevGov scanners、Doctor、dashboard refreshes、tests 與 reports 仍維持 proposal-only，不寫入真實 memory。
...
也可以用同一個 gate 啟動已審查（reviewed）的 raw command。只有 registry 與 TCP availability checks 通過後才會執行子程序；子程序會從 registry 取得 `HOST`、`PORT`、`DEVGOV_HOST`、`DEVGOV_PORT` 等環境變數：

```powershell
npm run port:preflight -- --project my-app --service web-http -- npm run dev:raw
```

若不需要啟動 registry entry，只需臨時檢查 TCP 可用性時：
...
其他 registry 也遵守同一原則：canonical 檔案只存穩定 ID 與已審查 policy。本機路徑、完整啟動命令、Terminal settings 路徑、Cloudflare credential 路徑與暫時掃描證據都留在 `reports/`。
