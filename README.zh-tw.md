...
`DevGov` 是容易記憶的本機多專案開發治理工具包與儀表板。它把開發服務、本地端服務 Agents、Windows Terminal profiles、開機啟動項、Cloudflare 對外路由、worktrees、自我檢測與本機文件檢索放在可稽核的 source of truth 後面管理。
...
`npm run doctor` 會驗證 package identity、registry schemas、dashboard port allocation、startup governance records、必要 scripts、dashboard port availability，以及文件索引能否建立。報告會寫入 `reports/devgov-doctor-report.json`。
