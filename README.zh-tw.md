...
Shared resource coordination 會記錄在 `registry/resource-coordination.registry.json`。它定義多個並行 LLM 開發工作可共用的輕量 communication / read-model surface，包括 lag diagnosis、freshness rules，以及 browser profiles、GPU-heavy rendering、foreground screen control 等排他資源的使用前登記。預設 scan 是 on-demand 且很小；scheduling 仍是日後需要另行 review 的 apply path。
