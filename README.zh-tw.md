...
`DevGov` 是 user/home-level governance toolkit 與儀表板。它把 Global AGENTS planning、開發服務、本地端服務 Agents、Windows Terminal profiles、開機啟動項、Cloudflare 對外路由、開發用 API key 存放位置、AGENTS 指令治理、worktrees、自我檢測與本機文件檢索放在可稽核的 source of truth 後面管理。
...
| 服務 | Host | Port | 說明 |
|---|---|---:|---|
| dashboard-http | `127.0.0.1` | `3101` | 長期使用的 loopback 儀表板。若 port 被占用，server 會直接失敗，不會靜默換 port。 |
| governance-public-http | `127.0.0.1` | `3000` | 受 Zero Trust 保護的 `gov.colorgeek.co` origin。Startup 透過已 review 的 on-demand helper 維持。 |

## 儀表板與啟動機制

儀表板入口是 `http://127.0.0.1:3101`。它直接讀取 canonical registry files，並提供 `/health`、`/api/state`、`/api/local-agents` 與 `/api/doctor` 作為本機檢查端點。
...
公開的 `gov.colorgeek.co` route 也採 review-gated。`npm run gov:route:register` 會建立使用者 Startup entry，讓 `127.0.0.1:3000` origin 與專用 Cloudflare Tunnel connector 在登入後保持可用；`npm run gov:route:remove` 可移除該 entry。
