...
真實 memory update 需要另一個 reviewed gate。產生 proposal 後、使用任何 runtime-approved memory update mechanism 前，先使用 `templates/CODEX.memory.rcg-update-gate.md`。
...
接著在任何 explicit memory-update workflow 前，先 review `templates/CODEX.memory.rcg-update-gate.md`。
